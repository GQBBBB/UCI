...
from ChestXray14_dataset import ChestXray14_train, ChestXray14_val
import timeit, time
...
def get_arguments():
    ...
    parser.add_argument("--snapshot_dir", type=str, default='snapshots/withChestXray14/')
    ...
    # 2D
    parser.add_argument("--data_dir2D", type=str, default='../Chestxray/image/')
    parser.add_argument("--label_path2D", type=str, default='./data_list/Chestxray14/')
    parser.add_argument("--input_size2D", type=str, default='224,224')
    parser.add_argument("--batch_size2D", type=int, default=32)
    parser.add_argument("--num_classes2D", type=int, default=14)
...
def main():
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.learning_rate, eps=1e-4, weight_decay=3e-5)
    print(optimizer)
    ...
    loss_cls = torch.nn.BCEWithLogitsLoss().cuda()
    ...
    # data loader
    train_set2D = ChestXray14_train(img_dir=args.data_dir2D, label_file=osp.join(args.label_path2D, "train_label.csv"))
    trainloader2D = torch.utils.data.DataLoader(train_set2D, batch_size=args.batch_size2D, shuffle=True, num_workers=8, pin_memory=True)#, pin_memory=True
    print(f"2D Training Data loaded: there are {len(train_set2D)} images.")
    val_set2D = ChestXray14_val(img_dir=args.data_dir2D, label_file=osp.join(args.label_path2D, "val_label.csv"))
    ...
    for epoch in range(real_num_epochs):
        ...
        for it, (batch2D, batch3D) in tqdm(enumerate(zip(trainloader2D, trainloader3D))):
            ...
            with torch.cuda.amp.autocast():
                ...
                epoch_loss_cls.append(loss_su2D.cpu().data.numpy())
                epoch_loss_seg.append(loss_su3D.cpu().data.numpy())
                epoch_loss.append(term_all.cpu().data.numpy())
...
def val_mode_cls(dataloader, model, proto2D, proto3D, epoch):
    ...
    for index, batch in tqdm(enumerate(dataloader)):
        ...
        pro_score.append(torch.nn.Sigmoid()(pred).cpu().data.numpy())
        label_val.append(label.data.numpy())

    pro_score = np.concatenate(pro_score, 0)
    binary_score = np.float32(pro_score>=0.5)
    ...
    for i in range(NUM_CLASSES):
        val_acc, val_auc, val_AP = cla_evaluate(label_val[:,i], binary_score[:,i], pro_score[:,i])
        val_acc_total += val_acc
        val_auc_total += val_auc
        val_AP_total += val_AP
    
    val_acc_total /= NUM_CLASSES
    val_auc_total /= NUM_CLASSES
    val_AP_total /= NUM_CLASSES

    return val_acc_total, val_auc_total, val_AP_total
